...
def startup():
    try:
        ...
        # Default Employee Account (EMP001 / 123456)
...
async def check_in(latitude: float = Form(...), longitude: float = Form(...), type: str = Form(default="CHECK_IN"), accuracy: float = Form(default=0.0), file: UploadFile = File(...), db: Session = Depends(get_db)):
