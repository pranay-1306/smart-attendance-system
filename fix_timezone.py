...
def root():
    return {
        ...
        "docs": "https://smart-attendance-system-nmo4.onrender.com/docs"
    }
...
def get_employee_dashboard(emp_id: int, db: Session = Depends(get_db)):
    ...
    # Calculate day metrics using Indian Standard Time
    now_ist = datetime.now(IST)
    today_str = now_ist.strftime("%Y-%m-%d")
    today_logs = [l for l in logs if l.timestamp.strftime("%Y-%m-%d") == today_str]
    ...
    current_month_prefix = now_ist.strftime("%Y-%m")
    month_logs = [l for l in logs if l.timestamp.strftime("%Y-%m") == current_month_prefix]
    unique_present_days = len(set(l.timestamp.strftime("%Y-%m-%d") for l in month_logs))
...
async def check_in(
    ...
    if not matched_emp:
        raise HTTPException(status_code=401, detail=f"Biometric mismatch. Confidence: {conf}%. Please align face directly.")
        
    # Record punch in Indian Standard Time
    now_ist = datetime.now(IST)
    log = AttendanceLog(
    ...
    return {
        ...
        "confidence": conf,
        "timestamp": now_ist.strftime("%Y-%m-%d %I:%M:%S %p IST")
    }
